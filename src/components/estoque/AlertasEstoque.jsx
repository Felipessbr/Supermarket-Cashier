...
export default function AlertasEstoque({ produtos, onMovimentar }) {
  ...
  const produtosBaixos = Object.entries(produtos).filter(
    ([, p]) => p.estoque > 0 && p.estoque <= p.estoqueMinimo * 0.5
  );
  const produtosAtenção = Object.entries(produtos).filter(
    ([, p]) => p.estoque > p.estoqueMinimo * 0.5 && p.estoque <= p.estoqueMinimo
  );

  const totalAlertas =
    produtosCriticos.length + produtosBaixos.length + produtosAtenção.length;
      ...
      >
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`${cores[nivel]} text-white text-xs px-2 py-1 rounded-full font-bold`}
                >
                  {textos[nivel]}
                </span>
              </div>
              <p className="text-sm text-gray-400">
                Código: {codigo} • {produto.categoria}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 mb-4">
          <div className="bg-white/5 rounded-lg p-3 text-center">
            ...
            <p
              className={`text-2xl font-bold ${cores[nivel].replace(
                "bg-",
                "text-"
              )}`}
            >
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 rounded-xl font-bold hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:bg-white/20 transition-all ">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-2 m-2">
              <AlertTriangle className="w-6 h-6" />
              Alertas de Estoque
            </h2>
            <p className="text-[var(--cor-texto)]">
              {totalAlertas} produto{totalAlertas !== 1 ? "s" : ""} requer
              {totalAlertas !== 1 ? "em" : ""} atenção.
            </p>
          </div>
      ...
      {produtosBaixos.length > 0 && (
        ...
        {/* PRODUTOS ATENÇÃO */}
        {produtosAtenção.length > 0 && (
        <div>
            <h3 className="text-xl font-bold text-yellow-400 mb-4 flex items-center gap-2">
            <Package className="w-5 h-5" />
            Atenção Baixo ({produtosAtenção.length})
            </h3>
            <div className="space-y-3">
            {produtosAtenção.map(([codigo, produto]) =>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-12 border border-white/20 text-center">
          ...
          <p className="text-[var(--cor-texto)]">Todos os produtos estão com estoque adequado</p>
        </div>
      )}
    </div>
  );
